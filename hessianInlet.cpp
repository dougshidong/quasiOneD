    ...
    std::vector <MatrixXd> &ddRindWdW)
{
    ...
    const double inlet_total_T = flo_opts.inlet_total_T;
...
//
    ...
    std::vector <Matrix3d> ddwpdwdwp = ddWpdWdWp(gam, W, 0);

    MatrixXd temp(3, 3);
    for (int Ri = 0; Ri < 3; Ri++)
    {
        temp.setZero();
        temp += dwpdw1.transpose() * ddRindWdW[Ri].topLeftCorner(3, 3);
        for (int Wpi = 0; Wpi < 3; Wpi++)
        {
            temp += dRidWi(Ri, Wpi) * ddwpdwdwp[Wpi];
        }
        ddRindWdW[Ri].topLeftCorner(3, 3) = temp * dwpdw1;
    }

    // Get Transformation Matrices
    MatrixXd dwpdw2 = eval_dWpdW(gam, W[i1*3+0], W[i1*3+1]);
    ddwpdwdwp = ddWpdWdWp(gam, W, 1);

    for (int Ri = 0; Ri < 3; Ri++) {
        temp.setZero();
        temp += dwpdw2.transpose() * ddRindWdW[Ri].bottomRightCorner(3, 3);
        for (int Wpi = 0; Wpi < 3; Wpi++)
        {
            temp += dRidWd(Ri, Wpi) * ddwpdwdwp[Wpi];
        }
        ddRindWdW[Ri].bottomRightCorner(3, 3) = temp * dwpdw2;
    }

    for (int Ri = 0; Ri < 3; Ri++)
    {
        ddRindWdW[Ri].topRightCorner(3, 3) =
            dwpdw1.transpose() * ddRindWdW[Ri].topRightCorner(3, 3) * dwpdw2;
    }
    for (int Ri = 0; Ri < 3; Ri++)
    {
        ddRindWdW[Ri].bottomLeftCorner(3, 3) =
            dwpdw2.transpose() * ddRindWdW[Ri].bottomLeftCorner(3, 3) * dwpdw1;
    }
}
